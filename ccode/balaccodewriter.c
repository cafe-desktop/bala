...
vala_ccode_writer_new (const gchar* filename,
                       const gchar* source_filename)
{
	return vala_ccode_writer_construct (BALA_TYPE_CCODE_WRITER, filename, source_filename);
}
                        ...
                        gboolean write_version)
{
	...
	if (write_version) {
		...
		_tmp14_ = _tmp13_;
		_tmp15_ = g_strdup_printf ("/* %s generated by valac %s, the Vala compiler", _tmp14_, BALA_BUILD_VERSION);
		_g_free0 (_tmp11_);
		_tmp11_ = _tmp15_;
		_g_free0 (_tmp14_);
	} else {
