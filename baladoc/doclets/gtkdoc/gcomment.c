/* gcomment.c generated by balac, the Vala compiler
 * generated from gcomment.bala, do not modify */
...
gtkdoc_header_finalize (GtkdocHeader * obj)
{
	...
	_g_free0 (self->name);
	self->annotations = (_bala_array_free (self->annotations, self->annotations_length1, (GDestroyNotify) g_free), NULL);
	_g_free0 (self->value);
}
...
gtkdoc_gcomment_to_string (GtkdocGComment* self)
{
	...
	bala_list_sort (_tmp19_, (GCompareDataFunc) gtkdoc_header_cmp, NULL, NULL);
	{
		...
		while (TRUE) {
			...
			_tmp28_ = bala_list_get (_tmp27_, _header_index);
	...
	if (_tmp62__length1 > 0) {
		...
		_tmp65_ = _bala_g_strjoinv (", ", _tmp64_, (gint) _tmp64__length1);
		_tmp66_ = _tmp65_;
	...
	if (_tmp86_ > 0) {
		...
		g_string_append (_tmp87_, "\n *");
		{
			...
			while (TRUE) {
				...
				_tmp96_ = bala_list_get (_tmp95_, _version_index);
			...
			_bala_iterable_unref0 (_version_list);
		}
	}
	_tmp107_ = builder;
		...
		while (TRUE) {
			...
			if (g_strcmp0 (_tmp10_, "Deprecated") == 0) {
				...
				if (g_strcmp0 (_tmp15_, "Since") == 0) {
					...
					baladoc_error_reporter_simple_warning (reporter, "GtkDoc", "Unknown versioning tag '%s'", _tmp20_);
				}
			}
			_gtkdoc_header_unref0 (header);
		}
...
"sed in newly-written code. %s</para></warning>", _tmp24_, _tmp25_);
	...
	bala_list_sort (_tmp32_, (GCompareDataFunc) gtkdoc_header_cmp, NULL, NULL);
	...
	if (_tmp33_) {
		...
		g_string_append (_tmp38_, "<variablelist role=\"params\">");
		{
			...
			while (TRUE) {
				...
				_tmp47_ = bala_list_get (_tmp46_, _header_index);
...
gtkdoc_gcomment_finalize (GtkdocGComment * obj)
{
	...
	self->symbol_annotations = (_bala_array_free (self->symbol_annotations, self->symbol_annotations_length1, (GDestroyNotify) g_free), NULL);
	_bala_iterable_unref0 (self->headers);
	_g_free0 (self->brief_comment);
	_g_free0 (self->long_comment);
	_g_free0 (self->returns);
	self->returns_annotations = (_bala_array_free (self->returns_annotations, self->returns_annotations_length1, (GDestroyNotify) g_free), NULL);
