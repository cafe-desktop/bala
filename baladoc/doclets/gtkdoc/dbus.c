/* dbus.c generated by balac, the Vala compiler
 * generated from dbus.bala, do not modify */
                                  ...
                                  GtkdocDBusParameter* parameter)
{
	...
	bala_collection_add ((ValaCollection*) _tmp0_, parameter);
}
                              ...
                              gboolean link)
{
	...
	if (_tmp22_ > 0) {
		...
		_tmp25_ = bala_list_get (_tmp24_, 0);
	...
	}
	{
		...
		i = 1;
		{
			...
			while (TRUE) {
				...
				_tmp40_ = bala_list_get (_tmp39_, i);
                                  ...
                                  gpointer klass)
{
	...
	_tmp1_ = bala_array_list_new (GTKDOC_DBUS_TYPE_PARAMETER, (GBoxedCopyFunc) gtkdoc_dbus_parameter_ref, (GDestroyNotify) gtkdoc_dbus_parameter_unref, _tmp0_);
                                  ...
                                  GtkdocDBusMember* member)
{
	...
	member->iface = _tmp0_;
	_tmp1_ = self->methods;
	bala_collection_add ((ValaCollection*) _tmp1_, member);
}
                             ...
                             ValadocErrorReporter* reporter)
{
	...
	if (!gtkdoc_text_writer_open (_tmp13_)) {
		...
		baladoc_error_reporter_simple_error (reporter, "GtkDoc", "unable to open '%s' for writing", _tmp15_);
		result = FALSE;
                                 ...
                                 ValadocErrorReporter* reporter)
{
	...
	method_indent = 0;
	{
		...
		while (TRUE) {
			...
			_tmp7_ = _method_list;
			_tmp8_ = bala_list_get (_tmp7_, _method_index);
	...
	signal_indent = 0;
	{
		...
		while (TRUE) {
			...
			_tmp20_ = _sig_list;
			_tmp21_ = bala_list_get (_tmp20_, _sig_index);
...
"<synopsis>", _tmp43_);
		{
			...
			while (TRUE) {
				...
				_tmp52_ = bala_list_get (_tmp51_, _method_index);
...
"<synopsis>", _tmp62_);
		{
			...
			_sig_list = _tmp64_;
			_tmp65_ = _sig_list;
			_tmp66_ = bala_collection_get_size ((ValaCollection*) _tmp65_);
			_tmp67_ = _tmp66_;
			_sig_size = _tmp67_;
			_sig_index = -1;
			while (TRUE) {
				...
				_tmp70_ = _sig_list;
				_tmp71_ = bala_list_get (_tmp70_, _sig_index);
...
"</refsect1>", _tmp78_, _tmp79_);
	...
	_tmp81_ = bala_collection_get_size ((ValaCollection*) _tmp80_);
	_tmp82_ = _tmp81_;
	if (_tmp82_ > 0) {
		GString* _tmp83_;
		const gchar* _tmp84_;
...
"<title role=\"details.title\">Details</title>", _tmp84_);
		{
			...
			while (TRUE) {
				...
				_tmp93_ = bala_list_get (_tmp92_, _method_index);
...
"<title role=\"signals.title\">Signal Details</title>", _tmp116_);
		{
			...
			while (TRUE) {
				...
				_tmp124_ = _sig_list;
				_tmp125_ = bala_list_get (_tmp124_, _sig_index);
                                     ...
                                     gpointer klass)
{
	...
	_tmp0_ = g_direct_equal;
	_tmp1_ = bala_array_list_new (GTKDOC_DBUS_TYPE_MEMBER, (GBoxedCopyFunc) gtkdoc_dbus_member_ref, (GDestroyNotify) gtkdoc_dbus_member_unref, _tmp0_);
	self->methods = (ValaList*) _tmp1_;
	_tmp2_ = g_direct_equal;
	_tmp3_ = bala_array_list_new (GTKDOC_DBUS_TYPE_MEMBER, (GBoxedCopyFunc) gtkdoc_dbus_member_ref, (GDestroyNotify) gtkdoc_dbus_member_unref, _tmp2_);
...
gtkdoc_dbus_interface_finalize (GtkdocDBusInterface * obj)
{
	...
	_g_free0 (self->purpose);
	_g_free0 (self->description);
	_bala_iterable_unref0 (self->methods);
	_bala_iterable_unref0 (self->signals);
}
