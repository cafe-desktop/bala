/* filehelper.c generated by balac, the Bala compiler
 * generated from filehelper.bala, do not modify */
 ...
 */
gboolean
baladoc_copy_file (const gchar* src,
                   const gchar* dest)
{
	FILE* fsrc = NULL;
	FILE* _tmp0_;
 ...
 */
gboolean
baladoc_copy_directory (const gchar* src,
                        const gchar* dest)
{
			...
			file = _tmp3_;
			{
				...
				while (TRUE) {
					...
					if (g_file_test (_tmp12_, G_FILE_TEST_IS_DIR)) {
						...
						if (!baladoc_copy_directory (_tmp14_, _tmp15_)) {
							result = FALSE;
						...
						if (!baladoc_copy_file (_tmp16_, _tmp17_)) {
							result = FALSE;
							_g_free0 (dest_file_path);
							_g_free0 (src_file_path);
							_g_free0 (file);
 ...
 */
gboolean
baladoc_remove_directory (const gchar* rpath)
{
	GError* _inner_error0_ = NULL;
	gboolean result = FALSE;
			...
			entry = _tmp4_;
			{
				...
				while (TRUE) {
					...
					if (is_dir == TRUE) {
						...
						const gchar* _tmp12_;
						_tmp12_ = path;
						tmp = baladoc_remove_directory (_tmp12_);
						if (tmp == FALSE) {
							result = FALSE;
							_g_free0 (path);
							_g_dir_close0 (dir);
